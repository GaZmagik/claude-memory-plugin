/**
 * Tests for Suggest Links — core behaviour
 *
 * LLM-type tests live in suggest-links-llm.spec.ts
 * Security tests live in suggest-links-security.spec.ts
 */
...
describe('suggestLinks', () => {
  ...
  it('threads cosine similarity score to linkMemories for same-scope auto-link', async () => {
    ...
    await suggestLinks({ basePath: TEST_BASE, autoLink: true });
  ...
  it('does not pass similarity for cross-scope auto-link', async () => {
    ...
    await suggestLinks({ basePath: TEST_AGENT_BASE, autoLink: true, allScopes: true, agentName: 'test-agent' });
